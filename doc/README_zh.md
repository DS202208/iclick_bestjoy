...
iCLICK BESTJOY LFO 小飞碟网关是一个可以接收homeassistant的TCP指令，转发影音设备的红外，BLE蓝牙，射频RF433,RF315的遥控信号的影音网关设备。
...
下载并将 `custom_components/iclick_bestjoy` 文件夹复制到 Home Assistant 的 `config/custom_components` 文件夹下。

## 配置

[设置 > 设备与服务 > 添加集成](https://my.home-assistant.io/redirect/brand/?brand=iclick_LFO) > 搜索“`iCLICK BESTJOY LFO`” > 下一步 > 输入iCLICK LFO小飞碟的局域网IP地址，端口号默认是9999不要更改。
...
- 网络连接不上？
  ...
  如果iCLICK LFO小飞碟断开电源，重新插上电源。需要重启homeassistant，否则可能连接错误。
